...
function initGameView() {
   const game = initGame();
   const board = initBoardView(game);
   ...
   return {
      play: function () {
         ...
         do {
            ...
            board.showResult(propousalCombination);
         } while (!game.finished());
...
function initCombinationView() {
   ...
   return {
      getPropousalCombination: function () {
         ...
         do {
            propousalCombination = console.readString(
               "Propon una combinación: "
            );
...
function initSecretCombination() {
   ...
   return {
      getSecretCombination: function () {
         ...
         do {
            color =
               combinations.getPosibleColors()[parseInt(Math.random() * 6)];
...
function initBoardView(game) {
   ...
   return {
      ...
      showResult: function (propousalCombination) {
         console.writeln(
            `Intento nro: ${game.getAttemps()} ${game.getResult(
